...
jxcollecturlArr = [],jxcollectheaderArr = []
let jxcollecturl = $.getdata('jxcollecturl')
let jxcollectheader = $.getdata('jxcollectheader')
...
if ($.isNode()) {
  ...
  if (process.env.JXCOLLECTHEADER && process.env.JXCOLLECTHEADER.indexOf('#') > -1) {
    jxcollectheader = process.env.JXCOLLECTHEADER.split('#');
    console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.JXCOLLECTHEADER && process.env.JXCOLLECTHEADER.indexOf('\n') > -1) {
    jxcollectheader = process.env.JXCOLLECTHEADER.split('\n');
    console.log(`您选择的是用换行隔开\n`)
  } else {
    jxcollectheader = process.env.JXCOLLECTHEADER.split()
...
!(async () => {
  ...
  for (let i = 0; i < jxcollecturlArr.length; i++) {
    if (jxcollecturlArr[i]) {
      ...
      jxcollectheader = jxcollectheaderArr[i];
      $.index = i + 1;
      console.log(`\n开始【京喜工厂电力收取${$.index}】`)
      await collect()
      await showmsg()
...
async function collect(){
  return new Promise((resolve) => {
    let collect_url = {
      ...
      headers: JSON.parse(jxcollectheader)
    }
