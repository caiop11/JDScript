...
function GetCoin(homepageinfo) {
  return new Promise(async (resolve) => {
    try {
      ...
      $.log(`您还没有领取过金币，快去手动点击牛牛🐮，领取一次金币吧～`);
      $.logErr(e);
    }
    finally {
      resolve();
...
function taskListUrl(function_path, body, stk) {
  let url = `${JD_API_HOST}/newtasksys/newtasksys_front/${function_path}?_=${Date.now()}&source=jxmc&bizCode=jxmc${body}&_ste=1&sceneval=2&g_login_type=1&g_ty=ajax`;
  if (stk) {
    url += `&_stk=${stk}`;
  }
